#!/bin/bash
# Post-installation script for Linux Download Manager (LDM) - RPM package
...
FIREFOX_NATIVE_DIR="/usr/lib64/mozilla/native-messaging-hosts"
...
# Also try the 32-bit path for compatibility
FIREFOX_NATIVE_DIR_32="/usr/lib/mozilla/native-messaging-hosts"
if [ ! -d "$FIREFOX_NATIVE_DIR_32" ]; then
    mkdir -p "$FIREFOX_NATIVE_DIR_32"
fi

if [ -f "/usr/share/ldm/native-messaging/ldm-firefox.json" ]; then
    cp "/usr/share/ldm/native-messaging/ldm-firefox.json" "$FIREFOX_NATIVE_DIR_32/"
    chmod 644 "$FIREFOX_NATIVE_DIR_32/ldm-firefox.json"
fi
...
# SELinux contexts (for RHEL/CentOS with SELinux enabled)
if command -v selinuxenabled >/dev/null 2>&1 && selinuxenabled; then
    if command -v restorecon >/dev/null 2>&1; then
        restorecon -R /usr/bin/ldm* || true
        restorecon -R /usr/share/ldm || true
        restorecon -R /etc/ldm || true
    fi
fi

# Create systemd user service directory if systemd is available
if command -v systemctl >/dev/null 2>&1; then
    SYSTEMD_USER_DIR="/usr/lib/systemd/user"
    if [ -d "$SYSTEMD_USER_DIR" ] && [ -f "/usr/share/ldm/systemd/ldm.service" ]; then
        cp "/usr/share/ldm/systemd/ldm.service" "$SYSTEMD_USER_DIR/"
        chmod 644 "$SYSTEMD_USER_DIR/ldm.service"
...
echo ""
if command -v selinuxenabled >/dev/null 2>&1 && selinuxenabled; then
    echo "SELinux is enabled. File contexts have been restored."
    echo ""
fi
