...
public:
    explicit DownloadWorker(DownloadItem *item, QObject *parent = nullptr)
        : QObject(parent), m_item(item), m_manager(nullptr),
          m_reply(nullptr), m_file(nullptr), m_speedTimer(nullptr),
...
public slots:
    void startDownload() {
        ...
        // Create objects in the correct thread
        if (!m_manager) {
        ...
        request.setHeader(QNetworkRequest::UserAgentHeader, "LDM/1.0.1 (Like Download Manager)");
...
private slots:
    ...
    void onDownloadFinished() {
        ...
        if (m_reply && m_reply->error() == QNetworkReply::NoError && !m_paused) {
            if (m_file) {
                m_file->close();
            }
            ...
            QString errorMsg = m_reply ? m_reply->errorString() : "Unknown error";
...
public:
    ...
    void startDownload(int id) {
        ...
        connect(worker, &DownloadWorker::paused, this, [this, id]() {
            // Worker is still active when paused
        });

        connect(worker, &DownloadWorker::resumed, this, [this, id]() {
            // Worker resumed
        });

        connect(worker, &DownloadWorker::cancelled, this, [this, id]() {
            m_workers.remove(id);
    ...
    void pauseDownload(int id) {
        if (m_workers.contains(id)) {
            QMetaObject::invokeMethod(m_workers[id], "pauseDownload", Qt::QueuedConnection);
        }
    }

    void resumeDownload(int id) {
        if (m_workers.contains(id)) {
            QMetaObject::invokeMethod(m_workers[id], "resumeDownload", Qt::QueuedConnection);
        } else {
            startDownload(id);
        }
    }
    ...
    void cancelDownload(int id) {
        if (m_workers.contains(id)) {
            QMetaObject::invokeMethod(m_workers[id], "cancelDownload", Qt::QueuedConnection);
        }
...
public:
    ...
    {
        setWindowTitle("LDM - Like Download Manager 1.0.1");
